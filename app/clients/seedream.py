    ...
    async def generate(
        ...
        if use_openai_compat:
            ...
            if width * height < min_pixels:
                ...
                logger.info(
                    "OpenAI compatible mode upsizing from %sx%s to %sx%s (maintaining aspect ratio)",
                    width,
                    height,
                    new_width,
                    new_height,
            ...
            if reference_images:
                ...
                if "image" in payload:
                    ...
                    if isinstance(img_val, list):
                        logger.info("Seedream payload has %d reference images prepared", len(img_val))
                    else:
                        logger.info("Seedream payload has 1 reference image prepared")
